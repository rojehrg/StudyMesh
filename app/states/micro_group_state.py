...
class MicroGroupState(rx.State):
    ...
    async def create_group(self):
        ...
        with rx.session() as session:
            ...
            session.commit()
        return [
            rx.toast.success("Group created successfully!"),
    ...
    async def join_group(self):
        ...
        with rx.session() as session:
            ...
            if not group:
                return rx.toast.error("Group not found.")
            ...
            if existing:
                return rx.toast.info("You are already in this group.")
            new_member = MicroGroupMember(group_id=group.id, user_id=auth_state.user_id)
            session.add(new_member)
            session.commit()
            self.join_group_code = ""
        return [rx.toast.success("Joined group!"), rx.redirect(f"/groups/{group.id}")]
    ...
    def generate_intro_message(self):
        ...
        if self.intro_message_template == "Casual":
            self.generated_intro_message = f"Hey everyone! 👋 I created a study group called '{self.current_group.name}' for {class_name}. Let's help each other ace this course! {link}"
        elif self.intro_message_template == "Serious":
            self.generated_intro_message = f"Hi team. I've organized '{self.current_group.name}' to focus on high-performance study sessions for {class_name}. Please review our goals. {link}"
        elif self.intro_message_template == "Exam Prep":
            self.generated_intro_message = f"📚 Exam prep group for {class_name} is now active! Join '{self.current_group.name}' so we can start reviewing material ASAP. {link}"
    ...
    async def remove_member(self, user_id: int):
        ...
        if (
            ...
            return rx.toast.error("Only the group creator can remove members.")
        with rx.session() as session:
            member = session.exec(
                select(MicroGroupMember).where(
            ...
            if member:
                ...
                if user_id == auth_state.user_id:
                    ...
                    return [
                        rx.toast.success("Member removed."),
