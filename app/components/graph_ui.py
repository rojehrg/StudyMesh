...
def student_node(data: rx.Var[dict], is_connectable: rx.Var[bool]):
    return rx.el.div(
        ...
        rx.el.div(
            ...
            class_name="w-10 h-10 rounded-full flex items-center justify-center mb-1 shadow-sm border border-gray-200",
            background_color=data["color"],
        ),
        rx.el.span(
            data["label"],
            class_name="text-[10px] font-medium text-gray-700 bg-white/90 px-1.5 py-0.5 rounded-full shadow-sm border border-gray-100 whitespace-nowrap max-w-[100px] truncate text-center",
        ),
            ...
            rx.radix.primitives.dialog.content(
                ...
                rx.el.div(
                    rx.radix.primitives.dialog.close(
                        rx.el.button(
                            "Close",
                            class_name="w-full mt-6 bg-indigo-600 text-white py-2 rounded-lg font-semibold hover:bg-indigo-700",
                        )
                    )
                ),
